...
bool sort_vendors(const ProductQueryResult::Bid &a, const ProductQueryResult::Bid &b) {
  return a.vendor_id_ < b.vendor_id_;
}
...
int main(int argc, char** argv) {
  ...
  if (argc == 3) {
    ...
    num_max_threads = std::min(20, std::max(1, atoi(argv[2])));
  }
  else if (argc == 2) {
    server_addr = std::string(argv[1]);
    num_max_threads = 4;
...
bool run_test(std::vector<ProductSpec>& product_specs, const int num_max_threads, const std::string& server_addr) {
  ...
  for (int i = 0; i < product_specs.size(); ++i) {
    ...
    threads_is_done[thread_ind] = false;
    
    threads[thread_ind] = new std::thread(thread_task, 
          server_addr, std::ref(product_specs[i]), std::ref(test_results[i]), i, std::ref(threads_is_done[thread_ind]));
  ...
  for (int i = 0; i < test_results.size(); ++i) {
    auto bids = test_results[i].bids_;
    if (bids.size()) {
      std::cout << "\n\nQuery id: " << i << ", product: " << product_specs[i].name_ << " is -->  ";
      std::sort(bids.begin(), bids.end(), sort_vendors);

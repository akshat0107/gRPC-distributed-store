...
class VendorService final : public Vendor::Service {
  ...
  public:
    VendorService(const std::string& vendor_name, const std::string& server_address)
      : id_(vendor_name + "|" + server_address) {}
...
void run_server(const std::string vendor_name, const std::string server_address)   {
  std::string server_addressess(server_address);
  VendorService service(vendor_name, server_address);
